...
def find_best_magnitude(eqns):
  best = -math.inf
  for i in range(len(eqns)):
    for j in range(i+1, len(eqns)):
      result_a = magnitude(add(expression(eqns[i]), expression(eqns[j])))
      result_b = magnitude(add(expression(eqns[j]), expression(eqns[i])))

      best = max(best, result_a, result_b)
